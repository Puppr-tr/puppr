...
import {DogGenderData} from '../../api/constant';
import StepIndicator from '../../components/Home/StepIndicator';
import {AuthNav} from '../../navigation/navigationKey';
import strings from '../../i18n/strings';

export default function DogProfileGender({navigation, route}) {
  ...
  const onPressNext = () => {
    if (select === '') {
      alert(strings.pleaseSelectDogGender);
    } else {
      navigation.navigate(AuthNav.DogProfileBreed, {
        ...userData,
        dogGender: select,
      });
    }
  ...
  return (
    <FSafeAreaView>
      ...
      <View style={localStyle.mainContainer}>
        ...
        <View>
          <StepIndicator step={3} rightIcon onPressNext={onPressNext} totalSteps={8} />
        </View>
      </View>
    </FSafeAreaView>
  );
